...
使用 `bd create` 发布 issue 到项目 beads 数据库。
...
### 5. Publish the issues as child beads

对每个批准的 slice，用一条命令发布子单：`bd create "标题" --parent <PRD_ID> --labels ready-for-agent --description "..."`（`--labels ready-for-agent` 使 sandcastle planner 能通过 `bd ready` 捡取）。使用下面的 issue body template。
